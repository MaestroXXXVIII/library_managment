...
class BorrowMapper:
    ...
    def entity_to_model(entity: BorrowEntity) -> BorrowModel:
        return BorrowModel(
            ...
            date_of_return=entity.date_of_return.value if entity.date_of_return is not None else None,
        )
    ...
    def model_to_entity(model: BorrowModel) -> tuple[BorrowEntity, str]:
        return (
            BorrowEntity(
                ...
                date_of_borrow=DateOfBorrow(model.date_of_borrow),
                date_of_return=DateOfReturn(model.date_of_return) if model.date_of_return  else None,
                borrow_id=model.id,
            ),
            model.book.title,
        )
