...
class ReturnBookInteractor:
    def __init__(self, borrow_repository: IBorrowRepository, increase_count_available_interactor: IncreaseCountAvailableInteractor):
        self._repository = borrow_repository
        self._increase_interactor = increase_count_available_interactor

    async def execute(self, borrow_id: int, return_date: date):
